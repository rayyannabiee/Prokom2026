...
public class peminjam extends manusia {
    ...
    // ── PERBAIKAN 1: cekNimAda() terpisah, tanpa print error ──
    private static boolean cekNimAda(String nim) {
        for (peminjam p : daftarpeminjam) {
            if (p.getnim().equals(nim)) return true;
        }
        return false;
    }
    ...
    public static boolean tambahpeminjam(peminjam Peminjam) {
        if (Peminjam.getnama() == null || Peminjam.getnim() == null || Peminjam.getnomorhp() == null) {
            System.out.println("Data tidak lengkap! Mohon isi secara lengkap.");
            return false;
        }
        // ── PERBAIKAN 2: return false jika NIM sudah ada ──
        if (cekNimAda(Peminjam.getnim())) {
            System.out.println("NIM " + Peminjam.getnim() + " sudah terdaftar.");
            return false;
        }
    ...
    // ── PERBAIKAN 3: carinim() tidak lagi print saat tidak ditemukan ──
    public static peminjam carinim(String nim) {
        for (peminjam p : daftarpeminjam) {
            if (p.getnim().equals(nim)) return p;
        }
        return null; // cukup return null, tanpa print
    }

    public static peminjam carinama(String nama) {
        for (peminjam p : daftarpeminjam) {
            if (p.getnama().equals(nama)) return p;
        }
        return null;
    }
    ...
    public static peminjam carinoHP(String nomorhp) {
        for (peminjam p : daftarpeminjam) {
            if (p.getnomorhp().equals(nomorhp)) return p;
        }
        return null;
    }
    ...
    protected static void dataKelas() {
        ...
        tambahpeminjam(new peminjam("Endry Julyan Putra",                "09021282530089", "08995268346",   0, "30-12-2026")); // hapus koma di NIM
        ...
        tambahpeminjam(new peminjam("M. Nabil Imtiyaz",                  "09021282530116", "082372335225",  0, "30-12-2026")); // hapus koma di NIM
        tambahpeminjam(new peminjam("M. Rayyan Alfarabi A.",             "09021282530096", "081278343489",  0, "30-12-2026")); // hapus tanda - di nomor hp
